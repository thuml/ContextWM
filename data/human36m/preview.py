...
for dataset in ['valid']:
    ...
    print(len(filelist), "\n")
    # part_center = part_center[-325:]
    # filelist = filelist[-325:]
    # part_center = part_center[:472]
    # filelist = filelist[:472]
    for file_name, center_poses in tqdm(zip(filelist, part_center)):
        file_name = file_name.strip()
        if not file_name.startswith("S11_Walking_1.58860488"):
            continue
...
PADDING = 200
ORINGIN_IMAGE_SIZE = 1000
OUTPUT_IMAGE_SIZE = 64

t = {}
for file, rect in tqdm(file_rect.items()):
    d = min(max(rect.w, rect.h) + PADDING, ORINGIN_IMAGE_SIZE)
    center = rect.center()
    rect = Rect(*(center - np.array([d, d]) / 2), *(center + np.array([d, d]) / 2))
    rect.fix_in_rect(Rect(0, 0, ORINGIN_IMAGE_SIZE, ORINGIN_IMAGE_SIZE))
    t[file] = rect
file_rect = t
