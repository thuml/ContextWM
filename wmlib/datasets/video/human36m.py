...
class Human(Dataset):
    def __init__(self, root_path, list_file, segment_len=50, image_tmpl='_{:06d}.jpg'):
    ...
    def _load_image(self, directory, idx):
        image = Image.open(os.path.join(self.root_path, directory +
                           self.image_tmpl.format((idx - 1) * 5 + 1))).convert('RGB')  # Modified
    ...
    def __getitem__(self, index):
        ...
        while not os.path.exists(os.path.join(self.root_path, record.path + self.image_tmpl.format(1))):  # Modified
            print(os.path.join(self.root_path, record.path + self.image_tmpl.format(1)))  # Modified
